...
function parseInitialVocabsResponse(text) {
...
export async function generateInitialVocabsViaGemini(input) {
    const apiKey = process.env.GEMINI_API_KEY;
    const modelName = process.env.GEMINI_MODEL; // ✅ Usar GEMINI_MODEL
    if (!apiKey)
        throw new Error("MISSING_GEMINI_API_KEY");
    if (!modelName)
        throw new Error("MISSING_GEMINI_MODEL_NAME"); // ✅ Lançar erro se o nome do modelo estiver faltando
    if (input.count !== 10) {
    ...
    const geminiResult = await callGeminiWithResilience(prompt, parseInitialVocabsResponse, // ✅ Usar o novo parser
    {
        // Contexto para logs
