...
import { generateExercise, type GeneratedExercise } from "../services/study/generateExercise.js";
import { evaluateExerciseViaGemini, type EvaluationResult } from "../services/ai/evaluateExercise.gemini.js"; // ✅ Importar EvaluationResult
...
export const studyRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/study/session", async (req, reply) => {
    ...
    const session = await prisma.studySession.create({
      data: {
        ...
        enabledExerciseTypes: body.enabledExerciseTypes as any,
        scope: body.scope,
        limit: body.limit,
  ...
  app.post("/study/exercises/:exerciseId/attempt", async (req, reply) => {
    try {
      ...
      const direction = (exercise.session?.direction ?? "WORD_TO_TRANSLATION") as
        | "WORD_TO_TRANSLATION"
        | "TRANSLATION_TO_WORD";

      // ✅ Usar o tipo EvaluationResult para 'ai'
      let ai: any;
      let evaluator: "DETERMINISTIC" | "GEMINI" = "DETERMINISTIC";
      ...
      const { attempt, review, progress } = await prisma.$transaction(async (tx) => {
        const attempt = await tx.studyAttempt.create({
          data: {
            ...
            userId: req.userId,
            response: responseValidated as any,
