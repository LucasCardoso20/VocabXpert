import { GoogleGenerativeAI, GenerativeModel, GoogleGenerativeAIFetchError, GenerationConfig } from "@google/generative-ai";
...
export async function callGeminiWithResilience<T>(
  prompt: string,
  parser: (text: string) => T, // Função para parsear a resposta da Gemini
  context: Record<string, any> = {}, // Contexto para logs
  generationConfig?: GenerationConfig // Importado de @google/generative-ai
): Promise<{ data: T | null; error: string | null; fallbackUsed: boolean }> {
