...
export function validateAttemptResponse(input: { exerciseType: string | undefined; response: unknown }) {
  ...
  if (t === "MULTIPLE_CHOICE_TRANSLATION") return IndexResponse.parse(input.response);
  if (t === "CLOZE") return IndexResponse.parse(input.response);
  if (t === "CHOOSE_CORRECT_EXAMPLE") return IndexResponse.parse(input.response);

  if (t === "FLASHCARD") return FlashcardResponse.parse(input.response);
  if (t === "WORD_ORDER") return WordOrderResponse.parse(input.response);
  if (t === "DICTATION") return DictationResponse.parse(input.response);
